...
public class Get {
	...
	public String getGroup(Player player) {
		String str = config.getString("Player."+player.getName()+".CR");
	...
	public String getPassword(String chatroom) {
		...
		for (int i = 0; i < player.length; i++)
			if (config.getString("Player."+player[i].getName()+".CR").equalsIgnoreCase(chatroom))
				return config.getString("Player."+player[i].getName()+".PW");
		return "n/a";
	}//close getPassword
	...
	public void getCurrentRooms(CommandSender sender) {
		...
		Player [] player = Bukkit.getServer().getOnlinePlayers();
		for (int i = 0; i < player.length; i++) {
			if (current.contains(config.getString("Player."+player[i].getName()+".CR"))) {
				}
			else {
				if (new isPrivate().IsPrivate(config.getString("Player."+player[i].getName()+".CR")))
					current.add(ChatColor.LIGHT_PURPLE+config.getString("Player."+player[i].getName()+".CR"));
				else current.add(ChatColor.DARK_AQUA+config.getString("Player."+player[i].getName()+".CR"));
				}
		}//close for
	...
	public ChatColor getCC(String player) {
		String color = config.getString("Player."+player+".CC");
