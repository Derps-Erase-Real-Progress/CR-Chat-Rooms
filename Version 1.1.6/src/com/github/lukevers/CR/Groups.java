...
public class Groups {
	...
	public void sendtoGroup(String group, String msg, String player) {
		...
		for (int i = 0; i < players.length; i++) {
			if (config.getString("Player."+players[i].getName()+".CR").equalsIgnoreCase(group)) {
	...
	public void showGroupMembers (String group, CommandSender sender) {
		...
		for (int i = 0; i < player.length; i++) {
			if (config.getString("Player."+player[i].getName()+".CR").equalsIgnoreCase(group)) {
				if (config.getString("Player."+player[i].getName()+".NM").equalsIgnoreCase(player[i].getName()))
					players.add(player[i].getName());
				else players.add(player[i].getName()+" ("+config.getString("Player."+player[i].getName()+".NM")+")");
			}
		}//close for
