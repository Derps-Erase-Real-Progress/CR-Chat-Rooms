	...
	public CMD_Private (int length, CommandSender sender, String name, String [] args) {
		...
		if (sender.hasPermission(new Permissions().publicprivate)) {
			if (name.equalsIgnoreCase("Console"))
				...
				if (config.getString("Player."+name+".CR").equalsIgnoreCase("Default")) {
					Bukkit.getServer().getPluginManager().getPlugin("CR").reloadConfig();
					if (length == 3) {
						config.set("Player."+sender.getName()+".PR", "true");
						config.set("Player."+sender.getName()+".PW", args[2]);
						config.set("Player."+sender.getName()+".CR", args[1]);
						sender.sendMessage(ChatColor.GRAY+"You have set the chat room "+ChatColor.WHITE+config.getString("Player."+name+".CR")+ChatColor.GRAY+" to "
								+ChatColor.LIGHT_PURPLE+"private" +ChatColor.GRAY+ " with the password of "+ChatColor.WHITE+config.getString("Player."+name+".PW"));
						}//close length of 3
					else if (length == 2) {
						config.set("Player."+sender.getName()+".PR", "true");
						config.set("Player."+sender.getName()+".PW",args[1]);
						sender.sendMessage(ChatColor.GRAY+"You have set the chat room "+ChatColor.WHITE+config.getString("Player."+name+".CR")+ChatColor.GRAY+" to "
								+ChatColor.LIGHT_PURPLE+"private" +ChatColor.GRAY+ " with the password of "+ChatColor.WHITE+config.getString("Player."+name+".PW"));
						}//close length of 2
					else if (length == 1) {
				...
				else if (config.getString("Player."+name+".PR").equalsIgnoreCase("true"))
