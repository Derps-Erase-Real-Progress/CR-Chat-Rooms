	...
	public CMD_Color (CommandSender sender, String name, String [] args, ArrayList <String> colors) {
		...
		if (args.length == 1) {
			...
			if (colors.contains(args[1].toUpperCase())) {
				if (config.getConfig().getString("BANNED."+args[1].toUpperCase()).equalsIgnoreCase("false")) {
					config.getConfig().set("Player."+name+".CC", args[1]);
			...
			if (sender.hasPermission(new Permissions().BAN_color_PERM)) {
				if (colors.contains(args[2].toUpperCase())) {
					config.getConfig().set("BANNED."+args[2].toUpperCase(), "true");
			...
			if (sender.hasPermission(new Permissions().BAN_color_PERM)) {
				if (colors.contains(args[2].toUpperCase())) {
					config.getConfig().set("BANNED."+args[2].toUpperCase(), "false");
			...
			if (sender.hasPermission(new Permissions().SET_DEFAULT_color)) {
				if (colors.contains(args[2].toUpperCase())) {
					config.getConfig().set("DEFAULT_color", args[2]);
